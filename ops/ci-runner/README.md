...
Template conversion publishes the complete read-only image atomically and refuses to replace an existing image. Conversion helpers mount only build inputs and outputs, with no network or operator private key. If template removal fails, the error names the container: remove it successfully with `docker rm -f <container>` before deleting its retained `template-build-*` directory. Rotating the operator SSH key requires rebuilding the golden image so its baked-in authorized key is replaced; install the rebuilt image only after draining guests using the old backing file.
