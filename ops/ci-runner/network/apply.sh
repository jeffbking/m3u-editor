...
iptables -w 10 -nL CI5900-FORWARD >/dev/null 2>&1 || iptables -w 10 -N CI5900-FORWARD
iptables -w 10 -nL CI5900-INPUT >/dev/null 2>&1 || iptables -w 10 -N CI5900-INPUT
add() { iptables -w 10 -C "$@" 2>/dev/null || iptables -w 10 -A "$@"; }
...
for subnet in 10.89.0.0/24 10.90.0.0/24; do
    iptables -w 10 -C DOCKER-USER -s "$subnet" -j CI5900-FORWARD 2>/dev/null || iptables -w 10 -I DOCKER-USER 1 -s "$subnet" -j CI5900-FORWARD
    iptables -w 10 -C INPUT -s "$subnet" -j CI5900-INPUT 2>/dev/null || iptables -w 10 -I INPUT 1 -s "$subnet" -j CI5900-INPUT
done
